...
public:
    ...
    // Set and update the state and price
    void SetState(double new_price, InquiryState new_state);
...
template<typename T>
void Inquiry<T>::SetState(double new_price, InquiryState new_state)
{
    price=new_price;
    state=new_state;
    return;}
